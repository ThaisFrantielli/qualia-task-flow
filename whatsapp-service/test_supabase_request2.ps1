...
$urls = @(
    "$supabase/rest/v1/clientes?select=*&status_triagem=eq.aguardando&order=created_at.asc",
    "$supabase/rest/v1/public.clientes?select=*&status_triagem=eq.aguardando&order=created_at.asc"
)
foreach ($url in $urls) {
    Write-Output "\nRequesting: $url"
    try {
