...
const ALLOWED_TABLES = new Set([
  'dim_frota', 'dim_veiculos', 'dim_contratos_locacao', 'dim_movimentacao_patios',
...
function buildContratosQuery(_fields?: string[]): string {
  ...
  return `
    SELECT 
      ...
      f.*, 
      f."DataCompra" AS "DataCompra",
      f."GrupoVeiculo" AS "Categoria",
      f."kminformado" AS "KmInformado",
      f."kmconfirmado" AS "KmConfirmado",
      f."IdadeEmMeses" AS "IdadeEmMeses",
      f."IdadeEmMeses" AS "IdadeVeiculo",
      f."ValorAtualFIPE" AS "ValorFipe",
      -- Effective end date (COALESCE DataFinal, DataEncerramento) and precomputed days to vencimento
      COALESCE(c."DataFinal", c."DataEncerramento") AS "DataFimEfetiva",
      EXTRACT(EPOCH FROM (COALESCE(c."DataFinal", c."DataEncerramento") - NOW())) / 86400 AS "DiasParaVencimento",
