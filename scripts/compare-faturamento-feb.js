...
function usage() {
  console.log('Uso: node scripts/compare-faturamento-feb.js --source source.csv --dashboard dashboard.csv --year 2026 --month 2 --expected 7018456.22');
  process.exit(1);
}
...
function parseCSV(content) {
  // Very small CSV parser: splits by lines and commas or semicolons, handles quoted values
...
function normalizeNumber(v) {
  ...
  // Remove currency symbols and thousands separator; accept both 1.234.567,89 and 1,234,567.89
...
function isCanceled(row) {
  // common fields: SituacaoNota, IdSituacaoNota, Situacao, Status
  const s = (row.SituacaoNota || row.Situacao || row.Status || row.IdSituacaoNota || '').toString().toLowerCase();
  // consider canceled if contains 'cancel' or equals 'cancelada' or IdSituacaoNota is a cancel code (e.g., 3)
  if (s.includes('cancel')) return true;
  if (s.includes('cancelada')) return true;
  if (s === '3') return true; // heuristic
  return false;
}
...
function isLocacaoEmitidoValid(row) {
  // Power BI rule: for Locação only include when IdSituacaoNota in {1,2} and FA/ND == 'FA'
  // Be permissive: if either field is missing, do not exclude.
...
function getValorByTipo(row) {
  // Prefer specific columns, fallback to ValorTotal
...
async function main() {
  ...
  if (args.source === 'db' || args.source === 'database') {
    // fetch from Postgres using env vars: PGHOST, PGUSER, PGPASSWORD, PGDATABASE, PGPORT
    ...
    try {
      ...
      src = res.rows.map(r => {
        // normalize keys to match CSV headers if possible
        const out = {};
  ...
  // show top 10 docs by value delta between sources
  const mapDashRows = {};
