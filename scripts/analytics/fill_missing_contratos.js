...
async function run() {
  ...
  try {
    ...
    const backupContratos = `public.backup_dim_contratos_locacao_${ts.replace(/[^a-zA-Z0-9_]/g,'')}`;
    const backupFrota = `public.backup_dim_frota_${ts.replace(/[^a-zA-Z0-9_]/g,'')}`;
...
FROM public.dim_contratos_locacao;`;
    ...
    fs.writeFileSync(path.join(backupDir, `counts_before_${ts}.json`), JSON.stringify(before, null, 2));
    console.log('Contagens antes salvas em', path.join('tmp', `counts_before_${ts}.json`));
    console.log(before);
    ...
    fs.writeFileSync(path.join(backupDir, `sample_missing_placa_${ts}.json`), JSON.stringify(samplePl.rows, null, 2));
    fs.writeFileSync(path.join(backupDir, `sample_missing_datas_${ts}.json`), JSON.stringify(sampleDs.rows, null, 2));
...
RETURNING c."ContratoLocacao" AS contrato, c."PlacaPrincipal" AS placa, f."Modelo" AS novo_modelo;`;
    ...
    fs.writeFileSync(path.join(backupDir, `updated_modelo_${ts}.json`), JSON.stringify(upd1.rows, null, 2));
...
RETURNING c."ContratoLocacao" AS contrato, c."PlacaPrincipal" AS placa, f."ValorAtualFIPE" AS novo_valor_fipe;`;
    ...
    fs.writeFileSync(path.join(backupDir, `updated_fipe_${ts}.json`), JSON.stringify(upd2.rows, null, 2));
...
RETURNING c."ContratoLocacao" AS contrato, c."PlacaPrincipal" AS placa, f."kminformado" AS novo_km;`;
    ...
    fs.writeFileSync(path.join(backupDir, `updated_km_${ts}.json`), JSON.stringify(upd3.rows, null, 2));
...
RETURNING f."Placa" AS placa, f."Id" AS frota_id, c."ValorAtualFIPE" AS novo_valor_fipe;`;
    ...
    fs.writeFileSync(path.join(backupDir, `updated_frota_fipe_${ts}.json`), JSON.stringify(upd4.rows, null, 2));

    // Contagens depois
    const resCountsAfter = await client.query(countsSql);
    const after = resCountsAfter.rows[0];
    fs.writeFileSync(path.join(backupDir, `counts_after_${ts}.json`), JSON.stringify(after, null, 2));
    console.log('Contagens depois salvas em', path.join('tmp', `counts_after_${ts}.json`));
