...
import { Car, AlertTriangle, Activity, Filter, X } from 'lucide-react';
...
export default function FleetDashboard(): JSX.Element {
  ...
  const hasActiveFilters = useMemo(() => {
    return !!(filterState.status || filterState.modelo || filterState.ageRange);
  }, [filterState]);

  const clearFilters = () => {
    setFilterState({ status: null, modelo: null, ageRange: null });
    setPage(1);
  };
  ...
  const kpis = useMemo(() => {
    ...
    const ociosos = filteredData.filter(r => r.Status !== 'Locado' && r.Status !== 'Em Operação').length;
    const percOciosidade = total > 0 ? (ociosos / total) * 100 : 0;

    return { total, patrimonioFipe, custoAquisicao, idadeMedia, percOciosidade };
  }, [filteredData]);
  ...
  const statusData = useMemo(() => {
    ...
    filteredData.forEach(r => {
      const s = r.Status || r.Situacao || 'Indefinido';
      map[s] = (map[s] || 0) + 1;
    });
    return Object.entries(map).map(([name, value]) => ({ name, value }));
  }, [filteredData]);

  // 2. BarChart: Top 10 Modelos
  ...
  const tableData = useMemo(() => {
    return filteredData.map(r => {
      ...
      return {
        ...
        fipeAtual: fipeAtual,
        variacao: variacao,
        idade: parseNum(r.IdadeVeiculo || r.IdadeMeses)
      };
    });
  }, [filteredData]);
  ...
  const handleStatusClick = (data: any) => {
    setFilterState(prev => ({ ...prev, status: prev.status === data.name ? null : data.name }));
    setPage(1);
  };
    ...
    <div className="bg-slate-50 min-h-screen p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <Text className="mt-1 text-slate-500">
            Análise completa da frota com interatividade total. Clique nos gráficos para filtrar.
          </Text>
        </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {filterState.status && (
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                ...
                <X className="w-3 h-3 cursor-pointer" onClick={() => setFilterState(prev => ({ ...prev, status: null }))} />
              </span>
            )}
            {filterState.modelo && (
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                Modelo: <strong>{filterState.modelo}</strong>
                <X className="w-3 h-3 cursor-pointer" onClick={() => setFilterState(prev => ({ ...prev, modelo: null }))} />
              </span>
            )}
            {filterState.ageRange && (
              <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                Idade: <strong>{filterState.ageRange} meses</strong>
                <X className="w-3 h-3 cursor-pointer" onClick={() => setFilterState(prev => ({ ...prev, ageRange: null }))} />
              </span>
            )}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card decoration="top" decorationColor="amber" className="bg-white border border-slate-200 shadow-sm">
          ...
          <Metric className="text-slate-900">{kpis.idadeMedia.toFixed(1)} meses</Metric>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
          ...
          <DonutChart
            ...
            className="h-60"
            onVolumeChange={(v: any) => v && handleStatusClick(v)}
          />
        </Card>
        ...
        <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <Text className="text-slate-500 text-sm mb-4">
            {filterState.ageRange ? `Filtrado: ${filterState.ageRange} meses` : 'Clique nas barras para filtrar'}
          </Text>
          <div className="h-60">
      ...
      <Card className="bg-white shadow-sm border border-slate-200">
        ...
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-slate-500">
            Mostrando {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, tableData.length)} de{' '}
            {tableData.length}
          </div>
          <div className="flex items-center gap-2">
            <button
