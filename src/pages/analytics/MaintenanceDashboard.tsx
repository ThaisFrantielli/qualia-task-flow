        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={typeData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" cursor="pointer" onClick={(d, _, e) => handleChartClick('tipo', d.name, e as unknown as React.MouseEvent)}>
                    {typeData.map((entry, i) => (<Cell key={i} fill={isValueSelected('tipo', entry.name) ? '#b45309' : ['#f59e0b', '#ef4444', '#3b82f6', '#10b981', '#8b5cf6', '#64748b'][i % 6]} stroke={isValueSelected('tipo', entry.name) ? '#78350f' : 'none'} strokeWidth={isValueSelected('tipo', entry.name) ? 2 : 0}/>))}
                  </Pie>
            ...
            <div className="h-72 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={typeData} layout="vertical">
                  ...
                  <Bar dataKey="value" radius={[0,4,4,0]} barSize={18} cursor="pointer" onClick={(d, _, e) => handleChartClick('tipo', d.name, e as unknown as React.MouseEvent)}>
                    {typeData.map((entry) => (<Cell key={entry.name} fill={isValueSelected('tipo', entry.name) ? '#b45309' : '#f59e0b'} />))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="h-80 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topFornecedores} layout="vertical">
                  ...
                  <Bar dataKey="valor" radius={[0,4,4,0]} barSize={18} cursor="pointer" onClick={(d, _, e) => handleChartClick('oficina', d.name, e as unknown as React.MouseEvent)}>
          ...
          {fornecedores.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {fornecedores.slice(0, 6).map((f, idx) => (
                <Card key={idx} className={`cursor-pointer hover:shadow-md transition-all ${isValueSelected('oficina', f.Nome) ? 'ring-2 ring-amber-500 bg-amber-50' : ''}`} onClick={(e) => handleChartClick('oficina', f.Nome, e)}>
                  <Text className="font-medium truncate">{f.NomeFantasia || f.Nome}</Text>
                  <div className="flex items-center gap-4 mt-2"><Metric className="text-amber-600">{f.TotalOS || 0}</Metric><Text className="text-slate-500">ordens de serviço</Text></div>
          ...
          <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 sticky top-0">
                <tr><th className="p-3">Data</th><th className="p-3">Placa</th><th className="p-3">Modelo</th><th className="p-3">Fornecedor</th><th className="p-3">Tipo</th><th className="p-3 text-right">Dias</th><th className="p-3 text-right">Valor</th></tr>
              </thead>
              <tbody>
                {pageItems.map((r, idx) => (
                  <tr key={idx} className="border-t hover:bg-slate-50">
                    <td className="p-3">{r.DataEntrada ? new Date(r.DataEntrada).toLocaleDateString('pt-BR') : '-'}</td>
                    <td className={`p-3 font-mono cursor-pointer hover:text-amber-600 ${isValueSelected('placa', r.Placa) ? 'text-amber-600 font-bold' : ''}`} onClick={(e) => handleChartClick('placa', r.Placa, e)}>{r.Placa}</td>
                    <td className="p-3 truncate max-w-[120px]">{r.Modelo || '-'}</td>
                    <td className={`p-3 truncate max-w-[150px] cursor-pointer hover:text-amber-600 ${isValueSelected('oficina', r.Fornecedor) ? 'text-amber-600 font-bold' : ''}`} onClick={(e) => handleChartClick('oficina', r.Fornecedor, e)}>{r.Fornecedor || '-'}</td>
                    <td className={`p-3 truncate max-w-[100px] cursor-pointer hover:text-amber-600 ${isValueSelected('tipo', r.TipoManutencao) ? 'text-amber-600 font-bold' : ''}`} onClick={(e) => handleChartClick('tipo', r.TipoManutencao, e)}>{r.TipoManutencao || '-'}</td>
