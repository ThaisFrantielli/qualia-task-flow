// src/pages/Configuracoes/ConfiguracoesEquipes/HierarquiaTab.tsx
import React, { useState } from 'react';
import { Users, Plus, Trash2, UserCheck, ArrowRight, Edit } from 'lucide-react';
...
const HierarquiaTab: React.FC = () => {
  ...
  const handleAddMember = () => {
    if (selectedUserId) {
      // permite escolher supervisor no formulário (se houver selecionado)
      addMemberMutation.mutate(selectedUserId, {
        onSuccess: async (data: any) => {
          // se foi selecionado um supervisor diferente do usuário atual, atualiza
          try {
            const supervisorToUse = selectedSupervisorId || user?.id || '';
            if (supervisorToUse && supervisorToUse !== (user?.id || '')) {
              // data.id é o id do registro de user_hierarchy recém-criado
              await updateSupervisorMutation.mutateAsync({ hierarchyId: data.id, newSupervisorId: supervisorToUse });
            }
          } catch (e) {
            // erros serão tratados pelos hooks; não bloquear a UX
          }
  ...
  // Editar supervisor de um membro
  ...
  const openEditSupervisorDialog = (item: any) => {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Visualize sua estrutura organizacional</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Hierarquia e Equipe</h2>
          <p className="text-gray-500 text-sm">Configure a hierarquia e gerencie sua equipe</p>
        </div>
                ...
                <div key={member.id} className="flex items-center gap-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => openEditSupervisorDialog(member)}
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione um usuário para adicionar à sua equipe. Escolha também o supervisor (padrão: você).
            </DialogDescription>
          </DialogHeader>
                  ...
                  {availableUsers.map(u => (
                    <SelectItem key={u.id} value={u.id}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">({u.nivelAcesso || 'Usuário'})</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleUpdateSupervisor} disabled={updateSupervisorMutation.isPending}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
