...
const tabs = ["Atividades", "Pós-Vendas", "Detalhes", "Anexos", "Resolução", "Comentários", "Notificações", "Pesquisas", "Responsável", "Equipe"];
...
const CustomerDetail: React.FC<CustomerDetailProps> = ({
  ...
  resolucao,
  onEdit,
  // onCreateAtendimento, // removed unused prop
  status,
  step,
  activities,
  tab,
  ...
  // Tickets de pós-venda do Supabase
  ...
  // const [showAtividadeModal, setShowAtividadeModal] = React.useState(false); // removed unused state
    return (
      <section className="col-span-6 bg-white rounded-lg shadow p-6 flex flex-col">
        ...
        {tab === "Pós-Vendas" && (
          <div className="flex-1 overflow-y-auto">
            {loadingPosVendas ? (
              <div className="text-gray-400">Carregando tickets...</div>
            ) : showTicket === null ? (
              <div className="flex flex-col gap-3">
                {posVendasTickets.length === 0 && <div className="text-gray-400">Nenhum ticket de pós-venda encontrado.</div>}
                {posVendasTickets.map((t) => (
                  <div key={t.id} className="border rounded p-3 hover:bg-gray-50 cursor-pointer" onClick={() => setShowTicket(t.id)}>
                    <div className="font-semibold text-blue-900">{t.summary || '-'}</div>
                    <div className="text-xs text-gray-500">{t.created_at ? new Date(t.created_at).toLocaleString() : '-'}</div>
                    <div className="text-sm mt-1">{t.final_analysis || '-'}</div>
                    <div className="text-xs mt-1"><b>Status:</b> {t.status || '-'}</div>
                  </div>
                ))}
              </div>
            ) : (
              (() => {
                ...
                return (
                  <div className="border rounded p-4 bg-gray-50">
                    <button className="mb-2 text-blue-600 underline text-xs" onClick={() => setShowTicket(null)}>&larr; Voltar para lista</button>
                    <h3 className="text-lg font-bold mb-2">{ticket.summary || '-'}</h3>
                    <div className="mb-2"><b>Resumo:</b> {ticket.summary || '-'}</div>
                    <div className="mb-2"><b>Análise Final:</b> {ticket.final_analysis || '-'}</div>
                    <div className="mb-2"><b>Detalhes da Resolução:</b> {ticket.resolution_details || '-'}</div>
                    <div className="text-xs text-gray-500">Criado em: {ticket.created_at ? new Date(ticket.created_at).toLocaleString() : '-'}</div>
                    <div className="text-xs mt-2"><b>Status:</b> {ticket.status || '-'}</div>
                  </div>
                );
              })()
            )}
    ...
    {tab === "Detalhes" && (
      <div className="flex flex-col gap-2 text-sm">
        ...
        <div><b>Departamento:</b> {department || '-'}</div>
        <div><b>Responsável:</b> {responsible || '-'}</div>
  <div><b>Criado em:</b> {createdAt ? new Date(createdAt as string).toLocaleString() : '-'}</div>
  <div><b>Atualizado em:</b> {updatedAt ? new Date(updatedAt as string).toLocaleString() : '-'}</div>
        <div><b>Origem:</b> {origem || '-'}</div>
        <div><b>Resumo:</b> {resumo || '-'}</div>
      </div>
    )}
    ...
    {tab === "Anexos" && (
      <div className="flex flex-col gap-2">
        {((Array.isArray(anexos) ? anexos : []) as { filename: string; url: string }[]).length > 0 ? (
          ((anexos ?? []) as { filename: string; url: string }[]).map((a, i) => (
            <a key={i} href={a.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">{a.filename}</a>
          ))
        ) : (
          <div className="text-gray-400">Nenhum anexo encontrado.</div>
      ...
      (() => {
        ...
        return (
          <div className="flex flex-col gap-2 items-start">
            <div className="flex items-center gap-2">
                  {profile?.avatar_url ? (
                    <img src={profile!.avatar_url ?? undefined} alt="avatar" className="w-10 h-10 rounded-full" />
                  ) : null}
                  <div>
                    <div className="font-semibold">{profile?.full_name}</div>
                    <div className="text-xs text-gray-500">{profile?.email}</div>
