          ...
          {activeStep === 'analysis' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm p-4 border rounded-lg bg-background">
                  ...
                  <div><Label>Motivo:</Label><Input value={editableData.reason} onChange={(e) => setEditableData(p => ({...p, reason: e.target.value}))} /></div>
                  <div><Label>Departamento:</Label><Select value={editableData.department} onValueChange={(v) => setEditableData(p => ({...p, department: v}))}><SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent>{departamentos.map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}</SelectContent></Select></div>
              </div>
            </div>
          )}
                ...
                <div className="space-y-2 mt-4">
                  ...
                  {delegatedTasks.length > 0 ? (
                    <ul className="border rounded-md divide-y">
                      {delegatedTasks.map(task => (<li key={task.id} className="p-3 flex justify-between items-center"><div><p className="font-medium">{task.title}</p><p className="text-sm text-muted-foreground">Para: {task.assignee_name || 'N/A'}</p></div><div className="text-sm capitalize">{task.status}</div></li>))}
                    </ul>
          ...
          {activeStep === 'resolution' && (
            <div className="space-y-4">
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-background">
                  <div><Label>Análise Final:</Label><Select value={editableData.final_analysis} onValueChange={(v) => setEditableData(p => ({...p, final_analysis: v}))}><SelectTrigger><SelectValue placeholder="Selecione..." /></SelectTrigger><SelectContent>{analisesFinais.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent></Select></div>
                  <div><Label>Texto da Resolução:</Label><Textarea value={editableData.resolution_details} onChange={(e) => setEditableData(p => ({...p, resolution_details: e.target.value}))} rows={5}/></div>
                </div>
            </div>
          )}
          
          {/* Aba de Histórico */}
          {activeStep === 'history' && (<div className="space-y-4"><h2 className="text-2xl font-semibold">Histórico de Comunicação</h2><p className="text-center p-4 text-sm text-muted-foreground">Em construção.</p></div>)}
