...
import { useUsers } from '@/hooks/useUsers'; // CORREÇÃO: O nome do hook é 'useUsers'
import type { Subtask, Profile } from '@/types'; // CORREÇÃO: Importa os tipos necessários
...
const ActionPlan: React.FC<ActionPlanProps> = ({ taskId }) => {
  const { subtasks, isLoading, add, update } = useSubtasks(taskId);
  const { users: profiles, loading: isLoadingProfiles } = useUsers(); // CORREÇÃO: Usa o hook correto
  const { register, handleSubmit, control, reset, formState: { isSubmitting } } = useForm<FormData>({
    defaultValues: { title: '', assignee_id: null }
  });
  ...
  // CORREÇÃO: O tipo do parâmetro é 'Subtask'
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select onValueChange={field.onChange} value={field.value || ""} disabled={isLoadingProfiles || isSubmitting}>
              <SelectTrigger className="w-[200px]"><SelectValue placeholder="Atribuir a..." /></SelectTrigger>
              <SelectContent>
                {/* CORREÇÃO: Adiciona o tipo explícito para 'p' */}
                {profiles.map((p: Profile) => <SelectItem key={p.id} value={p.id}>{p.full_name}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
        />
      ...
      {isLoading ? <div className="flex justify-center p-4"><Loader2 className="animate-spin text-primary" /></div> : (
        <div className="space-y-2">
          {subtasks.map(subtask => (
            <div key={subtask.id} className="flex items-center gap-3 p-2 border rounded-md hover:bg-muted/50 transition-colors">
              <Checkbox id={`subtask-${subtask.id}`} checked={subtask.completed} onCheckedChange={() => handleToggleComplete(subtask)} />
              <label htmlFor={`subtask-${subtask.id}`} className={`flex-grow cursor-pointer ${subtask.completed ? 'line-through text-muted-foreground' : ''}`}>
                {subtask.title}
              </label>
