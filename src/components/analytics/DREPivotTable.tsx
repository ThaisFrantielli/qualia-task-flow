...
interface DREPivotTableProps {
    ...
    displayOrder?: 'parent-first' | 'leaf-first';
}
...
export default function DREPivotTable({
    ...
    const renderNode = (node: AccountNode, depth: number = 0): JSX.Element[] => {
        ...
        const showCodeOnly = node.description.trim() === node.code.trim();
                    ...
                    <div className={`flex items-center gap-2 ${indentClass}`}>
                        ...
                        <div className="flex flex-col">
                            <span className={`text-sm ${textClass}`}>
                                {showCodeOnly ? node.code : node.description}
                            </span>
                            {node.level > 0 && !showCodeOnly && (
                                <span className="text-[10px] text-slate-500 font-normal">{node.code}</span>
                            )}
                        </div>
                    </div>
                </td>
        ...
        if (displayOrder === 'leaf-first') {
            rows.push(...childrenRows, currentRow);
        } else {
            rows.push(currentRow, ...childrenRows);
        }
